  ...
  template: `
    <div class="max-w-md mx-auto bg-white p-8 rounded-xl shadow-lg border border-gray-100 animate-fade-in">
      ...
      <div class="space-y-4">
        ...
        <input #emailInput type="text" [ngModel]="email" (ngModelChange)="email = $event.trim().toLowerCase()" (keydown.enter)="passwordInput.focus()" class="w-full rounded-lg border-gray-300 py-2 px-3 border" placeholder="Email">
        <input #passwordInput type="password" [(ngModel)]="password" (blur)="password = password.trim()" (keydown.enter)="isRegistering() ? register() : login()" class="w-full rounded-lg border-gray-300 py-2 px-3 border" placeholder="Password">
...
export class LoginUserComponent {
  ...
  register(){
    ...
    this.hotelService.register(cleanEmail, cleanPassword, "user").subscribe({
      next: (res:any) => {
  ...
  login(){
    ...
    this.hotelService.login(cleanEmail, cleanPassword, "user").subscribe({
      next: (res: any) => {
